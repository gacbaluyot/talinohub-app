      ...
      <div class="mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          <div class="flex items-center">
            ...
            <span class="ml-4 inline-flex items-center rounded-md bg-blue-100 text-blue-800 px-3 py-1 text-sm font-medium">
              ...
              >
                <MenuItems class="absolute right-0 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                  ...
                  <MenuItem v-slot="{ active }">
                    <a
                      href="#"
                      @click.prevent="navigateTo('/educator')"
                      :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']"
                    >
                      Dashboard
                    </a>
    ...
    <div v-else-if="course" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="bg-gradient-to-r from-indigo-600 to-purple-700 rounded-lg p-8 mb-8 text-white">
        ...
        <div class="mt-6 flex items-center gap-3">
          <button
            disabled
            class="inline-flex items-center rounded-md bg-white/50 px-6 py-3 text-base font-semibold text-white cursor-not-allowed"
          >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
          ...
          <div class="bg-white rounded-lg shadow p-6">
            ...
            <div v-else-if="lessons.length === 0" class="text-center py-8">
              ...
              <p class="mt-2 text-sm text-gray-500">No lessons available yet.</p>
            </div>
            ...
            <div v-else class="space-y-2">
              <div
                ...
                class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-indigo-300 cursor-pointer transition-all group"
              >
                <div class="flex items-center gap-4">
                  <div class="h-12 w-12 rounded-lg bg-indigo-100 flex items-center justify-center shrink-0 group-hover:bg-indigo-200">
                    <component :is="getLessonIcon(lesson.type)" class="h-6 w-6 text-indigo-600" />
                  </div>
                  <div>
                    <div class="flex items-center gap-2">
                      <span class="text-sm font-medium text-gray-500">Lesson {{ index + 1 }}</span>
                      <span v-if="lesson.is_preview" class="inline-flex items-center rounded-full bg-green-100 text-green-800 px-2 py-0.5 text-xs font-medium">
                        Preview
                      </span>
                    </div>
                    <p class="font-semibold text-gray-900 group-hover:text-indigo-600">{{ lesson.title }}</p>
        ...
        <div class="lg:col-span-1 space-y-6">
          <!-- Instructor Info -->
          <div v-if="course.educator" class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Instructor</h3>
            <div class="flex items-center gap-3 mb-4">
              <div class="h-16 w-16 rounded-full bg-indigo-100 flex items-center justify-center">
                <UserCircleIcon class="h-10 w-10 text-indigo-600" />
              </div>
              <div>
                <p class="font-semibold text-gray-900">{{ course.educator.name }}</p>
                <p class="text-sm text-gray-500">{{ course.educator.email }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-4">
              <div class="flex items-center justify-between">
                ...
                <span class="font-semibold text-gray-900">{{ course.lessons_count || 0 }}</span>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 text-gray-600">
                  <AcademicCapIcon class="h-5 w-5" />
                ...
                <span class="font-semibold text-gray-900">{{ course.level || 'Any' }}</span>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 text-gray-600">
                  <GlobeAltIcon class="h-5 w-5" />
                ...
                <span class="font-semibold text-gray-900">{{ course.language || 'English' }}</span>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 text-gray-600">
                ...
                <span class="font-semibold text-gray-900">{{ course.is_free ? 'Free' : `$${course.price}` }}</span>
              </div>
            </div>
          </div>
        </div>
