...
import type { BlogUpdateRequest } from '~/types/blogs'
...
const formData = ref<BlogUpdateRequest>({
  title: '',
  content: ''
})
...
const initQuill = async () => {
  if (editorContainer.value && !quillInstance && process.client) {
    ...
    if (blogsStore.blog) {
      formData.value.title = blogsStore.blog.title
      formData.value.content = blogsStore.blog.content
      quillInstance.root.innerHTML = blogsStore.blog.content
    }
  }
}
...
// Load blog and initialize Quill when modal opens
...
const handleSubmit = async () => {
  if (!formData.value.title || !formData.value.content || !props.blogUuid) {
    return
  }

  try {
    const updatedBlog = await blogsStore.updateBlog(props.blogUuid, formData.value)
    
    // Update the blog in the list with the returned blog data
    const index = blogsStore.blogs.findIndex(b => b.uuid === props.blogUuid)
    if (index !== -1 && updatedBlog) {
      blogsStore.blogs[index] = updatedBlog
    }
