  ...
  <div>
          ...
          <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="-translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="-translate-x-full">
            <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
              ...
              <div class="relative flex grow flex-col gap-y-5 overflow-y-auto bg-white px-6 pb-4 dark:bg-gray-900 dark:ring dark:ring-white/10 dark:before:pointer-events-none dark:before:absolute dark:before:inset-0 dark:before:bg-black/10">
                <div class="relative flex h-16 shrink-0 items-center gap-x-3">
                  <img class="h-8 w-auto dark:hidden" src="https://tailwindcss.com/plus-assets/img/logos/mark.svg?color=indigo&shade=600" alt="TalinoHub" />
                  <img class="h-8 w-auto not-dark:hidden" src="https://tailwindcss.com/plus-assets/img/logos/mark.svg?color=indigo&shade=500" alt="TalinoHub" />
                  <span class="text-xl font-semibold text-surface-900 dark:text-surface-0 dark:text-white">TalinoHub</span>
                </div>
                <nav class="relative flex flex-1 flex-col">
                  <ul role="list" class="flex flex-1 flex-col gap-y-7">
                    <li>
                      <ul role="list" class="-mx-2 space-y-1">
                        <li v-for="item in navigation" :key="item.name">
                          <NuxtLink :to="item.href" :class="[item.current ? 'bg-gray-50 text-indigo-600 dark:bg-white/5 dark:text-white' : 'text-surface-700 dark:text-surface-300 hover:bg-gray-50 hover:text-indigo-600 dark:text-surface-400 dark:text-surface-500 dark:hover:bg-white/5 dark:hover:text-white', 'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold']">
                            <component :is="item.icon" :class="[item.current ? 'text-indigo-600 dark:text-white' : 'text-surface-400 dark:text-surface-500 group-hover:text-indigo-600 dark:group-hover:text-white', 'size-6 shrink-0']" aria-hidden="true" />
                            {{ item.name }}
                          </NuxtLink>
                        </li>
                      </ul>
                    ...
                    <li class="mt-auto">
                      <a href="#" class="group -mx-2 flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold text-surface-700 dark:text-surface-300 hover:bg-gray-50 hover:text-indigo-600 dark:text-gray-300 dark:hover:bg-white/5 dark:hover:text-white">
                        <Cog6ToothIcon class="size-6 shrink-0 text-surface-400 dark:text-surface-500 group-hover:text-indigo-600 dark:group-hover:text-white" aria-hidden="true" />
                        Settings
                      </a>
                    </li>
                  </ul>
    ...
    <div class="lg:pl-72">
      <div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-xs sm:gap-x-6 sm:px-6 lg:px-8 dark:border-white/10 dark:bg-gray-900 dark:shadow-none">
        <button type="button" class="-m-2.5 p-2.5 text-surface-700 dark:text-surface-300 hover:text-surface-900 dark:text-surface-0 lg:hidden dark:text-surface-400 dark:text-surface-500 dark:hover:text-white" @click="sidebarOpen = true">
        ...
        <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
          ...
          <div class="flex items-center gap-x-4 lg:gap-x-6">
            <button type="button" class="-m-2.5 p-2.5 text-surface-400 dark:text-surface-500 hover:text-surface-500 dark:text-surface-400 dark:hover:text-white">
            ...
            <Menu as="div" class="relative">
              <MenuButton class="relative flex items-center">
                ...
                <span class="hidden lg:flex lg:items-center">
                  <span class="ml-4 text-sm/6 font-semibold text-surface-900 dark:text-surface-0 dark:text-white" aria-hidden="true">{{ user?.name || 'User' }}</span>
                  <ChevronDownIcon class="ml-2 size-5 text-surface-400 dark:text-surface-500 dark:text-surface-500 dark:text-surface-400" aria-hidden="true" />
                </span>
              </MenuButton>
              <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform scale-100" leave-to-class="transform opacity-0 scale-95">
                <MenuItems class="absolute right-0 z-10 mt-2.5 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg outline-1 outline-gray-900/5 dark:bg-gray-800 dark:shadow-none dark:-outline-offset-1 dark:outline-white/10">
                  <MenuItem v-slot="{ active }">
                    <button @click="handleLogout" :class="[active ? 'bg-gray-50 outline-hidden dark:bg-white/5' : '', 'block w-full text-left px-3 py-1 text-sm/6 text-surface-900 dark:text-surface-0 dark:text-white']">Sign out</button>
                  </MenuItem>
                </MenuItems>
              </transition>
            </Menu>
...
const navigation = computed(() => [
  ...
  { name: 'Blogs', href: '/dashboard/blogs/list', icon: UsersIcon, current: route.path.startsWith('/dashboard/blogs') },
])
